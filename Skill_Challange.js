...
function createMacros(macroname){
	...
	let API_SkillCha_Select_Skill = {
		...
		action: "!sc --skill ?{Select your skill|Acrobatics|Animal Handling|Arcana|Athletics|Deception|History|Insight|Intimidation|Investigation|Medicine|Nature|Perception|Performance|Persuasion|Religion|Sleight of Hand|Stealth|Survival}",
		istokenaction: false,
		visibleto: "all",
		_playerid: players[0].get("id")		
	}
	...
	let API_SkillCha_Set_Successes = {
		...
		action: "!sc --successes ?{Success Count}",
		istokenaction: false,
		visibleto: "",
		_playerid: players[0].get("id")		
	}
...
on("chat:message", function(msg){
	...
	if(msgArgs[0] === "!sc" && msgArgs[1] === "--startChallenge" && inChallenge === false){
		inChallenge = true;		
		setDc();
		setSuccesses();
	}	
	if(msgArgs[0] === "!sc" && msgArgs[1] === "--stopChallenge" && inChallenge === true){		
		resetVars();
	}
	if(msgArgs[0] === "!sc" && msgArgs[1] === "--showChallenge"){	
	...
	if(msgArgs[0] === "!sc" && msgArgs[1] === "--toggleDC"){
		dcHidden = !dcHidden;
		if(dcHidden){
	...
	if(msgArgs[0] === "!sc" && msgArgs[1] === "--dc"){
		currentDc = msgArgs[2];
	}
	if(msgArgs[0] === "!sc" && msgArgs[1] === "--successes"){
		maxSuccesses = msgArgs[2];
		maxFailures = Math.ceil(maxSuccesses/2);
	}
	if(msgArgs[0] === "!sc" && msgArgs[1] === "--skill"){
